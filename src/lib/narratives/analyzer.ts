...
export async function analyzeNarratives(
  ...
  const candidatePosts =
    posts.length > 600
      ? [...posts]
          .sort(
            (a, b) =>
              (b.likes ?? 0) + (b.replies ?? 0) - ((a.likes ?? 0) + (a.replies ?? 0)) ||
  ...
  if (clusterInput.length < 2) {
    return emptyResponse(posts.length, availablePlatforms, embeddingCoverage, options, sentimentCoverage, emotionCoverage);
  }
...
function buildFullNarrative(
  ...
  const engagement = chronological.reduce(
    (sum, p) => sum + (p.likes ?? 0) + (p.shares ?? 0) + (p.replies ?? 0),
    0
  );
