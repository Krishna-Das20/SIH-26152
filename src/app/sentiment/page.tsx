    ...
    <SkynetLayout>
      ...
      <main className="px-8 py-6">
        {loading ? (
                    ...
                    return (
                      <div key={item.name}>
                        <div className="flex justify-between mb-1.5">
                          <span className="text-[12px] text-skynet-text-secondary">{item.name}</span>
                          <span className="text-[12px] font-medium text-skynet-text-primary skynet-metric">
                            {item.value} posts ({pct}%)
                          </span>
                        </div>
                        <div className="h-2 bg-skynet-surface-secondary rounded-full overflow-hidden">
                ...
                <div className="space-y-3">
                  {(data.emotionRadar || [])
                    ...
                    .map((e: any) => (
                      <div
                        key={e.emotion}
                        className="flex items-center justify-between py-2 border-b border-skynet-border last:border-0"
                      >
                        <span className="text-[13px] text-skynet-text-secondary">{e.emotion}</span>
                        <div className="flex items-center gap-3">
                          <span className="text-[11px] text-skynet-muted">{e.rawCount} mentions</span>
                          <span className="text-[12px] font-medium text-skynet-text-primary skynet-metric w-12 text-right">
                            {e.value}%
                          </span>
                        </div>
                      </div>
              ...
              <div className="skynet-surface rounded-xl p-6">
                ...
                <div className="overflow-x-auto pt-4 pb-2">
                  <div className="flex items-center gap-2 min-w-[700px] h-40 relative px-2">
                    {/* Zero baseline */}
                    <div className="absolute left-0 right-0 top-1/2 h-px bg-skynet-border z-0" />
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="h-1/2 w-full flex items-end justify-center pb-0.5">
                            {isPositive && (
                              <div
                                className="w-full max-w-[18px] bg-skynet-positive/80 hover:bg-skynet-positive rounded-t-sm transition-all"
                          ...
                          <div className="h-1/2 w-full flex items-start justify-center pt-0.5">
                            {!isPositive && (
                              <div
                                className="w-full max-w-[18px] bg-skynet-negative/80 hover:bg-skynet-negative rounded-b-sm transition-all"
                                style={{ height: `${barHeight}px` }}
                                title={`${point.timestamp}: Score ${score.toFixed(2)} (${point.postVolume} posts)`}
                              />
                            )}
                          ...
                          {/* Timestamp label */}
                          <span className="absolute -bottom-5 text-[9px] font-mono text-skynet-muted whitespace-nowrap">
          ...
          <p className="text-skynet-muted text-sm">No sentiment data available.</p>
        )}
      </main>
    </SkynetLayout>
  );
