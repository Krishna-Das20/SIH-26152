        ...
        {loading ? (
          ...
          <>
            ...
            {topology.topKOLs && topology.topKOLs.length > 0 && (
              <div className="skynet-surface rounded-xl p-6 mb-8">
                <SectionHeader
                ...
                <div className="space-y-0">
                  {topology.topKOLs.map((kol: any, i: number) => (
                    <div
                      ...
                      className="flex items-center justify-between py-3.5 border-b border-skynet-border last:border-0 hover:bg-skynet-surface-secondary/30 skynet-transition px-2 rounded-lg"
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-[13px] text-skynet-text-primary font-medium">
                            {kol.displayName || kol.username || kol.id}
                          </p>
                          <div className="flex items-center gap-2 mt-0.5">
                            <span className="text-[10px] uppercase px-1.5 py-0.2 rounded bg-skynet-surface-secondary text-skynet-muted border border-skynet-border">
                              {kol.platform}
                            </span>
                            {kol.betweennessRank && (
                      ...
                      <div className="text-right flex items-center gap-4">
                        <div>
                          <span className="text-[13px] font-semibold text-skynet-text-primary skynet-metric">
                            {kol.influenceScore !== undefined ? kol.influenceScore : '—'}
                          </span>
                          <p className="text-[10px] text-skynet-muted">Influence Score</p>
                        </div>
                      </div>
                    </div>
                  ))}
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                  {topology.communities.slice(0, 8).map((comm: any, i: number) => {
                    ...
                    return (
                      <div
                        key={comm.id ?? i}
                        className="rounded-xl bg-skynet-surface-secondary/60 border border-skynet-border p-4 flex flex-col justify-between"
                      >
                        <div>
                          <div className="flex items-center justify-between mb-2">
                            <span className="skynet-label">Community {comm.id ?? i + 1}</span>
                            <span className="text-[10px] text-skynet-muted font-mono">
                              C0{i + 1}
                            </span>
                          </div>
                          <p className="text-2xl font-bold text-skynet-text-primary skynet-metric">
                            {memberCount}
                          </p>
                          <p className="text-[11px] text-skynet-text-secondary mt-0.5">
                            account nodes
                          </p>
                        </div>
                      </div>
          ...
          <div className="skynet-surface rounded-xl p-10 text-center">
            <NetworkIcon className="w-8 h-8 text-skynet-muted mx-auto mb-3" strokeWidth={1} />
            <p className="text-skynet-text-secondary text-sm">No network data available.</p>
          </div>
        )}
      </main>
    </SkynetLayout>
